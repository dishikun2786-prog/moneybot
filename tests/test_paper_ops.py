...
# 6.5 孤儿腿平仓 (close_perp_leg 之后)
r = paper_ops.close_orphan("BTCUSDT")
check("close_orphan 成功", r["ok"], r.get("error", ""))
st = json.load(open(os.path.join(TMP, "logs", "carry_state.json")))
check("孤儿腿已平", "BTCUSDT" not in st["orphans"])
r = paper_ops.close_orphan("BTCUSDT")
check("无孤儿时拒绝", not r["ok"])
...
# 13. 反向套利全链路 (空现货+多永续)
r = paper_ops.open_hedge("ETHUSDT", 10, "rev")
check("反向开仓", r["ok"] and "反向" in r["msg"], r.get("error", ""))
st = json.load(open(os.path.join(TMP, "logs", "carry_state.json")))
check("方向rev入库", st["positions"]["ETHUSDT"]["dir"] == "rev")
r = paper_ops.open_hedge("ETHUSDT", 10, "hack")
check("非法方向拒绝", not r["ok"])
# 反向转裸仓: 多头 sl<entry<tp (fixture perp=2750) → tp=2800(上), sl=2700(下)
r = paper_ops.close_spot_to_naked("ETHUSDT", 2800, 2700)
check("反向裸仓(多头校验)成功", r["ok"], r.get("error", ""))
st = json.load(open(os.path.join(TMP, "logs", "carry_state.json")))
check("反向裸腿入库", st["naked"]["ETHUSDT"]["dir"] == "rev")
r = paper_ops.close_spot_to_naked("BTCUSDT", 90000, 89000)
check("反向多头方向错误被拒(裸腿在ETH上,BTC无仓)", not r["ok"])
r = paper_ops.edit_naked_tpsl("ETHUSDT", 2690, 2810)
check("反向编辑TP/SL", r["ok"])
r = paper_ops.edit_naked_tpsl("ETHUSDT", 2810, 2690)
check("反向编辑方向错误被拒", not r["ok"])
r = paper_ops.close_naked("ETHUSDT")
check("平反向裸腿", r["ok"], r.get("error", ""))
