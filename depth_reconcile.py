...
def main():
    ...
    if g and p:
        ...
        if gb and pb and abs(float(gb[0])-float(pb[0])) > 0.5:
            problems.append(f"BTC bid0 偏差 {abs(float(gb[0])-float(pb[0])):.2f}")
        # px BTC last 偏差
        gl = g.get("px",{}).get("BTCUSDT",{}).get("last")
        pl = p.get("px",{}).get("BTCUSDT",{}).get("last")
        if gl and pl and abs(gl-pl) > 0.5:
            problems.append(f"BTC last 偏差 {abs(gl-pl):.2f}")
        # basis 偏差
