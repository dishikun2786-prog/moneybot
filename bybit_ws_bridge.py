...
# SPOT_CH_SYMS 动态生成: 见 load_ticker_syms 之后(现货通道标的 = 有现货的 bybit_syms 标的)
...
# 现货通道盘口/成交标的 = 动态现货清单(与 bybit_syms 同源, 自动含 TRX/XAUT)
SPOT_CH_SYMS = [s for s in SPOT_TICKER_SYMS if s in SYM_WHITELIST]
print(f"[bridge] 现货通道标的: {SPOT_CH_SYMS}", flush=True)
...
FUNDING_LOG = f"{BASE}/logs/carry_funding.jsonl"
FUNDING_PREV = {}  # sym -> 上次落盘的 funding_rate


def _funding_snapshot(sym):
    """分钟边界落盘 funding_rate（变化时追加，8h 结算周期去重）"""
    fr = PRICES.get(sym, {}).get("funding_rate")
    if fr is None:
        return
    fr = round(float(fr), 10)
    if FUNDING_PREV.get(sym) == fr:
        return
    FUNDING_PREV[sym] = fr
    _append_log(FUNDING_LOG, {"symbol": sym,
                              "funding_ts": int(time.time() * 1000),
                              "funding_rate": fr})
        ...
        with LOCK:
            ...
            if last_min and cur_min != last_min:
                for sym in DEPTH_SYMS:
                    ...
                    _funding_snapshot(sym)  # Phase F: funding 历史持续落盘(全标的)
            last_min = cur_min
            for sym in DEPTH_SYMS:
